 ...
 */
export async function voteAPI(tracePage, destPage, sectionID, text, summary) {
	let votedPageName = state.sectionTitles.find(x => x.data === sectionID)?.label || `section ${sectionID}`;
	mw.notify(`正在為「${votedPageName}」投出一票⋯⋯`);

	let res = await state.getApi().get({
		action: 'query',
		titles: destPage,
		prop: 'revisions|info',
		rvslots: '*',
